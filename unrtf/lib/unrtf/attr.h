...
UnRtfCollection *add_to_collection(UnRtfCollection *col, int nr, const char *text);
const char *get_from_collection(UnRtfCollection *c, int nr);
void free_collection(UnRtfCollection *c);

extern void attr_push(int attr, char *param, std::string& output, OutputPersonality *op);

extern void attrstack_push();
extern void attrstack_drop(std::string& output, OutputPersonality *op);
extern void attrstack_express_all(std::string& output, OutputPersonality *op);

extern int attr_find_pop(int findattr, std::string& output, OutputPersonality *op);
extern int attr_pop(int attr, std::string& output, OutputPersonality *op);

extern int attr_read();

extern void attr_drop_all();

extern void attr_pop_all(std::string& output, OutputPersonality *op);

extern void attr_pop_dump();

char *attr_get_param(int attr);

int safe_printf(int nr, std::string& output, char *string, ...);
