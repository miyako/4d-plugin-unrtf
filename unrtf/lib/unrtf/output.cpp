...
//#endif
		...
		result = (char *)get_alias(op, unicode); /* Try for ie, entity */
		if (result)
...
void
op_begin_std_fontsize(std::string& output, OutputPersonality *op, int size)
{
	...
	case 8:
		...
		{
			if (safe_printf(0, output, op->fontsize8_begin))
			{
	...
	case 10:
		...
		{
			if (safe_printf(0, output, op->fontsize10_begin))
			{
	...
	case 12:
		...
		{
			if (safe_printf(0, output, op->fontsize12_begin))
			{
	...
	case 14:
		...
		{
			if (safe_printf(0, output, op->fontsize14_begin))
			{
	...
	case 18:
		...
		{
			if (safe_printf(0, output, op->fontsize18_begin))
			{
	...
	case 24:
		...
		{
			if (safe_printf(0, output, op->fontsize24_begin))
			{
	...
	case 36:
		...
		{
			if (safe_printf(0, output, op->fontsize36_begin))
			{
	...
	case 48:
		...
		{
			if (safe_printf(0, output, op->fontsize48_begin))
			{
	...
	if (!found_std_expr)
	{
		if (op->fontsize_begin)
		{
			...
			if (safe_printf(1, output, op->fontsize_begin, expr))
			{
...
void
op_end_std_fontsize(std::string& output, OutputPersonality *op, int size)
{
	int found_std_expr = FALSE;
	...
	case 8:
		if (op->fontsize8_end)
		{
			if (safe_printf(0, output, op->fontsize8_end))
			{
	...
	case 10:
		...
		{
			if (safe_printf(0, output, op->fontsize10_end))
			{
	...
	case 12:
		...
		{
			if (safe_printf(0, output, op->fontsize12_end))
			{
	...
	case 14:
		...
		{
			if (safe_printf(0, output, op->fontsize14_end))
			{
	...
	case 18:
		...
		{
			if (safe_printf(0, output, op->fontsize18_end))
			{
	...
	case 24:
		...
		{
			if (safe_printf(0, output, op->fontsize24_end))
			{
	...
	case 36:
		...
		{
			if (safe_printf(0, output, op->fontsize36_end))
			{
	...
	case 48:
		...
		{
			if (safe_printf(0, output, op->fontsize48_end))
			{
	...
	if (!found_std_expr)
	{
		if (op->fontsize_end)
		{
            ...
            memset(expr, 0, sizeof(expr));/* clean buffer */
			snprintf(expr, 16, "%d", size);
			if (safe_printf(1, output, op->fontsize_end, expr))
			{
...
{
	op->aliases = (Aliases *)add_to_collection((UnRtfCollection *)op->aliases, nr, text);
}
...
{
	return get_from_collection((UnRtfCollection *)op->aliases, nr);
}
#endif
