...
word_new(char *str)
{
	...
	if (!w)
	{
		error_handler("cannot allocate a Word");
	}
...
static void
print_indentation(int level, std::string& output)
{
	int i;
...
void
word_dump(Word *w, std::string& output)
{
	const char *s;
...
//    printf("\n");
	indent_level += 2;
	print_indentation(indent_level, output);
...
/*========================================================================
 * Name:	optimize_word
 * Purpose:	Function tries to optimize group by eliminating redundant commands
 *   consecutively setting same attribute to same value.
...
optimize_word(Word *w, int depth)
{
	...
	if (depth > MAX_GROUP_DEPTH)
	{
		...
		warning_handler((char *)"Max group depth reached");
		return w;
	}
	for (; w != NULL; w = w->next)
	{
