...
static bool getUnRTFConf(std::string& path) {
    ...
    if(!hplugin) {
        /* Previously unchecked: passing NULL to GetModuleFileNameW below
           would silently resolve the *host executable's* path instead of
           the plugin's, pointing "Resources\" at the wrong folder. */
        return false;
    }
    
    DWORD len = GetModuleFileNameW(hplugin, thisPath, _MAX_PATH);
    if(len == 0 || len >= _MAX_PATH) {
        /* Previously unchecked: on failure thisPath stays all-zero
           (it's zero-initialized above), which _wsplitpath_s would
           happily split into empty components rather than erroring. */
        return false;
    }
    ...
    if(h) {
        ...
        if(getUnRTFConf(path)) {
            ...
            if(options) {
                ...
                double codepage_d = ob_get_n(options, L"codepage");
                if(std::isfinite(codepage_d) &&
                   codepage_d >= (double)INT_MIN &&
                   codepage_d <= (double)INT_MAX) {
                    codepage = (int)codepage_d;
                }
...
//                    }
            ...
            ob_set_s(status, L"error", "could not resolve plugin resources path");
        }
            ...
            if(f) {
                ...
                try {
                    ...
                    /* Catches word_read's std::invalid_argument as before,
                       ...
                       std::bad_alloc, std::out_of_range) it or word_print
                       might throw. Without this, an exception type other
                       than std::invalid_argument would unwind straight out
                       of UnRTF(), skip PA_ReturnObject below, and be
                       swallowed by PluginMain's catch(...) -- leaving the
                       4D host waiting on a return that never comes. */
                    ob_set_s(status, L"error", e.what());
                } catch (...) {
                    ob_set_s(status, L"error", "unknown error while parsing RTF");
                }

                if(word) {
                    word_free(word); /* word_read's tree was never freed before */
                }

                fclose(f);
            } else {
                ob_set_s(status, L"error", "could not open input buffer");
            }
            op_free(op); /* user_init's OutputPersonality was never freed before */
            PA_UnlockHandle(h);
        } else {
            ob_set_s(status, L"error", "could not initialize output personality (missing or bad .conf file)");
        }

    } else {
        ob_set_s(status, L"error", "missing input BLOB parameter");
    }
