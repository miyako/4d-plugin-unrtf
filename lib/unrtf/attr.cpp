...
extern void starting_body(std::string& output, OutputPersonality *op);
extern void starting_text(std::string& output, OutputPersonality *op);
...
static void attr_express_begin(int attr, char *param, std::string& output, OutputPersonality *op);
static void attr_express_end(int attr, char *param, std::string& output, OutputPersonality *op);
...
static void attr_pull_out(AttrStack *stack, int index, std::string& output, OutputPersonality *op)
{
	int j;
	/* Unexpress all newer attributes, and the one we'll remove */
	for (j = stack->tos; j >= index; j--)
	{
		attr_express_end(stack->attr_stack[j], stack->attr_stack_params[j], output, op);
	}
	...
	for (j = index; j <= stack->tos; j++) 
	{
		attr_express_begin(stack->attr_stack[j], stack->attr_stack_params[j], output, op);
	}
	return;
}

/* Unexpress and remove top attr. Internal version, no checking of args */
static void attr_pop_internal(AttrStack *stack, int attr, std::string& output, OutputPersonality *op)
{
	if (stack->tos >= 0)
	{
		char *param = stack->attr_stack_params[stack->tos];

		attr_express_end(attr, param, output, op);
...
static void attr_express_begin(int attr, char *param, std::string& output, OutputPersonality *op)
{
	...
	{
	case ATTR_BOLD:
		if (safe_printf(0, output, op->bold_begin))
		{
			fprintf(stderr, TOO_MANY_ARGS, "bold_begin");
		};
		break;
	case ATTR_ITALIC:
		if (safe_printf(0, output, op->italic_begin))
		{
	...
	case ATTR_WORD_UL:
	case ATTR_UNDERLINE:
		if (safe_printf(0, output, op->underline_begin))
		{
	...
	case ATTR_DOUBLE_UL:
		if (safe_printf(0, output, op->dbl_underline_begin))
		{
	...
	case ATTR_FONTSIZE:
		op_begin_std_fontsize(output, op, atoi(param));
		break;

	case ATTR_FONTFACE:
		if (safe_printf(1, output, op->font_begin, param))
		{
	...
	case ATTR_FOREGROUND:
		if (safe_printf(1, output, op->foreground_begin, param))
		{
	...
	case ATTR_BACKGROUND:
		if (!simple_mode)
			if (safe_printf(1, output, op->background_begin, param))
			{
	...
	case ATTR_SUPER:
		if (safe_printf(0, output, op->superscript_begin))
		{
			fprintf(stderr, TOO_MANY_ARGS, "superscript_begin");
		};
		break;
	case ATTR_SUB:
		if (safe_printf(0, output, op->subscript_begin))
		{
	...
	case ATTR_STRIKE:
		if (safe_printf(0, output, op->strikethru_begin))
		{
	...
	case ATTR_DBL_STRIKE:
		if (safe_printf(0, output, op->dbl_strikethru_begin))
		{
	...
	case ATTR_EXPAND:
		if (safe_printf(1, output, op->expand_begin, param))
		{
	...
	case ATTR_OUTLINE:
		if (safe_printf(0, output, op->outline_begin))
		{
			fprintf(stderr, TOO_MANY_ARGS, "outline_begin");
		};
		break;
	case ATTR_SHADOW:
		if (safe_printf(0, output, op->shadow_begin))
		{
			fprintf(stderr, TOO_MANY_ARGS, "shadow_begin");
		};
		break;
	case ATTR_EMBOSS:
		if (safe_printf(0, output, op->emboss_begin))
		{
			fprintf(stderr, TOO_MANY_ARGS, "emboss_begin");
		};
		break;
	case ATTR_ENGRAVE:
		if (safe_printf(0, output, op->engrave_begin))
		{
	...
	case ATTR_CAPS:
		...
		else
		{
			if (op->all_caps_begin)
				if (safe_printf(0, output, op->all_caps_begin))
				{
	...
	case ATTR_SMALLCAPS:
		...
		else
		{
			if (op->small_caps_begin)
				if (safe_printf(0, output, op->small_caps_begin))
				{
...
static void attr_express_end(int attr, char *param, std::string& output, OutputPersonality *op)
{
	...
	{
	case ATTR_BOLD:
		if (safe_printf(0, output, op->bold_end))
		{
			fprintf(stderr, TOO_MANY_ARGS, "bold_end");
		};
		break;
	case ATTR_ITALIC:
		if (safe_printf(0, output, op->italic_end))
		{
	...
	case ATTR_WORD_UL:
	case ATTR_UNDERLINE:
		if (safe_printf(0, output, op->underline_end))
		{
	...
	case ATTR_DOUBLE_UL:
		if (safe_printf(0, output, op->dbl_underline_end))
		{
	...
	case ATTR_FONTSIZE:
		op_end_std_fontsize(output, op, atoi(param));
		break;

	case ATTR_FONTFACE:
		if (safe_printf(0, output, op->font_end))
		{
	...
	case ATTR_FOREGROUND:
		if (safe_printf(0, output, op->foreground_end))
		{
	...
	case ATTR_BACKGROUND:
		if (!simple_mode)
			if (safe_printf(0, output, op->background_end))
			{
	...
	case ATTR_SUPER:
		if (safe_printf(0, output, op->superscript_end))
		{
			fprintf(stderr, TOO_MANY_ARGS, "superscript_end");
		};
		break;
	case ATTR_SUB:
		if (safe_printf(0, output, op->subscript_end))
		{
	...
	case ATTR_STRIKE:
		if (safe_printf(0, output, op->strikethru_end))
		{
	...
	case ATTR_DBL_STRIKE:
		if (safe_printf(0, output, op->dbl_strikethru_end))
		{
	...
	case ATTR_OUTLINE:
		if (safe_printf(0, output, op->outline_end))
		{
			fprintf(stderr, TOO_MANY_ARGS, "outline_end");
		};
		break;
	case ATTR_SHADOW:
		if (safe_printf(0, output, op->shadow_end))
		{
			fprintf(stderr, TOO_MANY_ARGS, "shadow_end");
		};
		break;
	case ATTR_EMBOSS:
		if (safe_printf(0, output, op->emboss_end))
		{
			fprintf(stderr, TOO_MANY_ARGS, "emboss_end");
		};
		break;
	case ATTR_ENGRAVE:
		if (safe_printf(0, output, op->engrave_end))
		{
	...
	case ATTR_EXPAND:
		if (safe_printf(0, output, op->expand_end))
		{
	...
	case ATTR_CAPS:
		...
		{
			if (op->all_caps_end)
				if (safe_printf(0, output, op->all_caps_end))
				{
	...
	case ATTR_SMALLCAPS:
		...
		else
		{
			if (op->small_caps_end)
				if (safe_printf(0, output, op->small_caps_end))
				{
...
void attr_push(int attr, char *param, std::string& output, OutputPersonality *op)
{
	AttrStack *stack = stack_of_stacks_top;
	int i;
	char *oldparam;
	...
	{
		attr_pull_out(stack, i, output, op);
	}
	...
	attr_express_begin(attr, param, output, op);
}

/* Unexpress and remove an attribute which is not the top one */
int attr_find_pop(int findattr, std::string& output, OutputPersonality *op)
{
	int i;
	AttrStack *stack = stack_of_stacks_top;
...
void
attrstack_unexpress_all(AttrStack *stack, std::string& output, OutputPersonality *op)
{
	int i;
	...
	for (i = stack->tos; i >= 0; i--)
	{
		attr_express_end(stack->attr_stack[i], stack->attr_stack_params[i], output, op);
	}
}
...
int
attr_pop(int attr, std::string& output, OutputPersonality *op)
{
	AttrStack *stack = stack_of_stacks_top;
	...
	{
		attr_pop_internal(stack, attr, output, op);
		return TRUE;
	}
	else
	{
...
void
attrstack_drop(std::string& output, OutputPersonality *op)
{
	AttrStack *stack = stack_of_stacks_top;
	if (!stack)
	{
	...
	{
		attrstack_express_all(output, op/*stack_of_stacks_top*/);
	}
}
...
void
attr_pop_all(std::string& output, OutputPersonality *op)
{
	AttrStack *stack = stack_of_stacks_top;
	if (!stack)
	{
	...
	while (stack->tos >= 0)
	{
		attr_pop_internal(stack, stack->attr_stack[stack->tos], output, op);
	}
}
...
void
attrstack_express_all(std::string& output, OutputPersonality *op)
{
	AttrStack *stack = stack_of_stacks_top;
	int i;
	...
	{
		attr_express_begin(stack->attr_stack[i], stack->attr_stack_params[i], output, op);
	}
}
...
int
safe_printf(int nr, std::string& output, char *string, ...)
{
...
UnRtfCollection *
add_to_collection(UnRtfCollection *col, int nr, const char *text)
{
	UnRtfCollection *c = col;

	if (col == NULL)
	{
		col = (UnRtfCollection *)my_malloc(sizeof(UnRtfCollection));
		col->nr = nr;
		col->text = text;
		col->next = NULL;
	}
	else
	{
		...
		c->next = (UnRtfCollection *)my_malloc(sizeof(UnRtfCollection));
		c->next->nr = nr;
		c->next->text = text;
		c->next->next = NULL;
	}
...
const char *
get_from_collection(UnRtfCollection *c, int nr)
{
	while (c != NULL)
	{
		if (c->nr == nr)
...
void
free_collection(UnRtfCollection *c)
{
	UnRtfCollection *c2;
